...
if __name__ == "__main__":
    ...
    train_input = [train_input_ori[0], train_input_ori[1], train_input_ori[2][:,-1*AMAZON_SHORT_SESS_MAX_LEN:], train_input_ori[3][:,-1*AMAZON_SHORT_SESS_MAX_LEN:], train_input_ori[4][:,-1*AMAZON_SIM_SESS_MAX_LEN:], train_input_ori[5][:,-1*AMAZON_SIM_SESS_MAX_LEN:]]
    print(train_input)
    print(train_label)
    print(train_input_ori[4].shape)
    print(np.sum(train_input_ori[4]>0))
    print(np.sum(train_input_ori[4][:,-1*AMAZON_SIM_SESS_MAX_LEN:]>0))

    test_size = len(test_input_ori[0])
    print("test_size:", test_size)
    test_input = [test_input_ori[0][:test_size],test_input_ori[1][:test_size], test_input_ori[2][:test_size,-1*AMAZON_SHORT_SESS_MAX_LEN:], test_input_ori[3][:test_size,-1*AMAZON_SHORT_SESS_MAX_LEN:], test_input_ori[4][:test_size,-1*AMAZON_SIM_SESS_MAX_LEN:], test_input_ori[5][:test_size,-1*AMAZON_SIM_SESS_MAX_LEN:]]
    test_label = test_label[:test_size]
    print("test info")
    print(test_input_ori[4].shape)
    print(np.sum(test_input_ori[4]>0))
    print(np.sum(test_input_ori[4][:,-1*AMAZON_SIM_SESS_MAX_LEN:]>0))
    BATCH_SIZE = 128
    ...
    model = SIM_LN_SOFTMAX01_V3(fd, fd, sess_feature, embedding_size=16, att_activation='dice',
                ...
                att_hidden_size=(80, 40,), hist_long_len_max=AMAZON_SIM_SESS_MAX_LEN)
