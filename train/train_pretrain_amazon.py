...
from config import RT_SESS_MAX_LEN, AMAZON_LONG_SESS_MAX_LEN, FRAC, TOP_K
from callback import roc_callback

SESS_MAX_LEN = AMAZON_LONG_SESS_MAX_LEN
long_hist_len_max=AMAZON_LONG_SESS_MAX_LEN
if len(sys.argv) > 1:
    if sys.argv[1] == "AADN":
        print('AADN')
...
if __name__ == "__main__":
    ...
    train_input = pd.read_pickle('../model_input_amazon/train_base.pkl')
    train_label = pd.read_pickle('../model_input_amazon/train_label.pkl')

    test_input = pd.read_pickle('../model_input_amazon/test_base.pkl')
    test_label = pd.read_pickle('../model_input_amazon/test_label.pkl')

    train_input = [train_input[0],train_input[1], train_input[2][:,-1*SESS_MAX_LEN:], train_input[3][:,-1*SESS_MAX_LEN:]]
    print("train_input", train_input)
    print("train_label", train_label)

    test_size = len(test_input[0])
    test_input = [test_input[0][:test_size],test_input[1][:test_size], test_input[2][:test_size, -1*SESS_MAX_LEN:], test_input[3][:test_size, -1*SESS_MAX_LEN:]]
