...
from models import DIN_LN_SOFTMAX01_V3
import numpy as np
import sys
...
if __name__ == "__main__":
    ...
    train_input = [train_input_ori[1], train_input_ori[2], train_input_ori[3][:,-1*DIN_SESS_MAX_LEN:], train_input_ori[4][:,-1*DIN_SESS_MAX_LEN:]]
    print(train_input)
    print(train_label)

    test_size = int(0.5 * len(test_input_ori[0]))
    print("test_size:", test_size)
    test_input = [test_input_ori[1][:test_size],test_input_ori[2][:test_size], test_input_ori[3][:test_size,-1*DIN_SESS_MAX_LEN:], test_input_ori[4][:test_size,-1*DIN_SESS_MAX_LEN:]]
    ...
    print('fd', fd)
    model = DIN_LN_SOFTMAX01_V3(fd, fd_hist, sess_feature, embedding_size=16, att_activation='dice',
