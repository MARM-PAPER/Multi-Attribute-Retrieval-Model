...
def MLP(unit_size, activation):
    layers = []
    for i in range(len(unit_size)):
        fc = Dense(unit_size[i],
                   activation=activation[i],
                   name='fc' + str(i),
                   kernel_initializer=RandomNormal(mean=0.0, stddev=0.01, seed=1024))
        layers.append(fc)
    return layers
...
def din(sparse_embedding_dict, sparse_input, seqs_input, feature_dim_dict, seq_feature_list, att_hidden_size,
        att_activation, att_weight_normalization):
    # target cid, iid
    ...
    # seqs cid, iid
    keys_emb_list = get_embedding_vec_list(sparse_embedding_dict, seqs_input, feature_dim_dict['sparse'],
                                           seq_feature_list, seq_feature_list)
...
def hard_search_loss(attr_scores, params):
    attr_num = params['attr_num']
    seqs_length = params['long_hist_len_max']

    print('attr_scores', attr_scores)

    # 二值化
    temperature = 0.001
    binary_score = tf.nn.sigmoid((attr_scores - 0.05) / temperature)  # 二值化  [batch*257, attr_num]
    topk_binary_score, topk_binary_attr = tf.math.top_k(binary_score[:5, 0, :], k=5)
    debug_info.append(["topk_binary_score", topk_binary_score])
    debug_info.append(["topk_binary_attr", topk_binary_attr])

    binary_score = tf.reshape(binary_score, [-1, 1 + seqs_length, attr_num])
    print('binary_score', binary_score)
    # target_attr = tf.tile(tf.reshape(binary_score[:, 0, :], [-1, 1, attr_num]), [1, seqs_length, 1])

    target_attr = tf.reshape(binary_score[:, 0, :], [-1, 1, attr_num])
    seqs_attr = binary_score[:, 1:, :]  # [batch_size, seqs, attr_num]

    search_score = tf.squeeze(tf.matmul(seqs_attr, target_attr, transpose_b=True), axis=-1)
    print('search_score', search_score)
    topk_search_score, topk_search_attr = tf.math.top_k(search_score[:5, :], k=5)
    debug_info.append(["topk_search_score", topk_search_score])
    debug_info.append(["topk_search_attr", topk_search_attr])
    total_length = tf.reduce_sum(tf.cast(search_score > 0.1, tf.float32), axis=-1)
    print('total_length', total_length)
    debug_info.append(['total_length', total_length])
    return search_score
...
def MASM(feature_dim_dict, seq_feature_list, embedding_size=8, attr_embedding_size=8,  rt_hist_len_max=100, long_hist_len_max=400,
    ...
    # din(sparse_embedding_dict, sparse_input, seqs_input, feature_dim_dict, seq_feature_list, att_hidden_size, att_activation, att_weight_normalization):
    # **********************masm************************************************
    # target cid, iid
