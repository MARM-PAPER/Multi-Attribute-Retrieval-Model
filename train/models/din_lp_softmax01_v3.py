...
def DINLP_LN_SOFTMAX01_V3(feature_dim_dict, hist_feature_dim_dict, seq_feature_list, embedding_size=8, hist_len_max=16, hist_long_len_max=256,
    ...
    hist = din_attention(query_emb, keys_emb, att_hidden_size, att_activation, seed, keys_mask, keys_emb.get_shape().as_list()[-1])
    hist_long = tf.reduce_mean(long_keys_emb, axis=1) #(None, 32)
    print("hist", hist)
...
def din_attention(queries, keys, att_hidden_size, att_activation, seed, mask, att_dnn_units):
    ...
    outputs = tf.nn.softmax(outputs/0.1)
