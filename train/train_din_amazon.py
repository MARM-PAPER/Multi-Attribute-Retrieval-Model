...
if __name__ == "__main__":
    ...
    train_input = [train_input_ori[0], train_input_ori[1], train_input_ori[2][:,-1*AMAZON_SHORT_SESS_MAX_LEN:], train_input_ori[3][:,-1*AMAZON_SHORT_SESS_MAX_LEN:]]
    print(train_input)
    print(train_label)

    test_size = len(test_input_ori[0])
    print("test_size:", test_size)
    test_input = [test_input_ori[0][:test_size],test_input_ori[1][:test_size], test_input_ori[2][:test_size,-1*AMAZON_SHORT_SESS_MAX_LEN:], test_input_ori[3][:test_size,-1*AMAZON_SHORT_SESS_MAX_LEN:]]
    ...
    print('fd', fd)
    model = DIN_LN_SOFTMAX01_V3(fd, fd, sess_feature, embedding_size=16, att_activation='dice',
