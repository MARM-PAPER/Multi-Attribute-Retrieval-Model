...
class roc_callback(Callback):
    ...
    def on_batch_end(self, batch, logs={}):
        if self.step % self.log_freq_b == 0:
        ...
        if self.step > 1000 and self.step % self.log_freq == 0:
            current_time = time.time()
            duration = current_time - self._start_time
            self._start_time = current_time
            examples_per_sec = self.log_freq / duration
            y_pred = self.model.predict(self.x)
            auc = roc_auc_score(self.y, y_pred)
            print('B_Time:', time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), ', Step #:', self.step,
                  ', Examples per second:', examples_per_sec, ' test auc: ', str(round(auc,4)))
        return
